...
void print_x86_string_region(FILE *f)
{
        ...
        if (t->kids[1] && t->kids[1]->leaf &&
            ...
            if (is_println) {
                ...
                if (arg_is_string && nargs == 1) {
                    /* println("some string") -- generate the string-literal
                     * address first, then pass that address as printf's
                     * format string.  This fixes the old Lab 10 bug where
                     * arg_addrs[0] was used before it was filled. */
                    struct tree *arg = (top > 0) ? stack[0] : NULL;
                    code = append(code, codegen(arg, current));
                    arg_addrs[0] = arg ? arg->place : ADDR_NONE;
 ...
 * Lab 11 repair pass
 * =========================================================================
 * Older Lab 10 println(expr) TAC sometimes became:
 *     addr loc:fmt,string:%d\n
 *     parm loc:fmt
 *     call printf,1,loc:ret
 * That calls printf("%d\n") without the integer argument, so the final
 * executable prints garbage.  This pass recognizes that pattern and inserts
 * the missing value parm before the format parm.
 */
static int same_addr(struct addr a, struct addr b)
{
    if (a.region != b.region) return 0;
    if (a.region == R_NAME)
        return a.u.name && b.u.name && strcmp(a.u.name, b.u.name) == 0;
    return a.u.offset == b.u.offset;
}

static int is_value_def_opcode(int op)
{
    return op == O_ASN || op == O_ADD || op == O_SUB || op == O_MUL ||
           op == O_DIV || op == O_NEG ||
           op == O_BLT || op == O_BLE || op == O_BGT || op == O_BGE ||
           op == O_BEQ || op == O_BNE;
}

static int is_int_printf_format_offset(int off)
{
    for (int i = 0; i < nstrings; i++) {
        if (string_offsets[i] == off && string_table[i] &&
            strcmp(string_table[i], "%d\n") == 0)
            return 1;
    }
    return 0;
}

static void fix_printf_numeric_parms(struct instr *code)
{
    struct addr last_value = ADDR_NONE;

    for (struct instr *l = code; l; l = l->next) {
        if (is_value_def_opcode(l->opcode) &&
            (l->dest.region == R_LOCAL || l->dest.region == R_GLOBAL)) {
            last_value = l->dest;
        }

        if (l->opcode == O_ADDR &&
            l->src1.region == R_STRING &&
            is_int_printf_format_offset(l->src1.u.offset) &&
            l->next && l->next->opcode == O_PARM &&
            same_addr(l->next->src1, l->dest) &&
            l->next->next && l->next->next->opcode == O_CALL &&
            l->next->next->dest.region == R_NAME &&
            l->next->next->dest.u.name &&
            strcmp(l->next->next->dest.u.name, "printf") == 0 &&
            l->next->next->src1.region == R_CONST &&
            l->next->next->src1.u.offset == 1 &&
            (last_value.region == R_LOCAL || last_value.region == R_GLOBAL) &&
            !same_addr(last_value, l->dest)) {

            struct instr *fmt_parm = l->next;
            struct instr *call = fmt_parm->next;
            struct instr *value_parm = gen(O_PARM, ADDR_NONE,
                                           last_value, ADDR_NONE);
            l->next = value_parm;
            value_parm->next = fmt_parm;
            call->src1 = ADDR_CONST(2);
        }
    }
}
