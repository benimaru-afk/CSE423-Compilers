...
typeptr typecheck(struct tree *t, SymbolTable current)
{
    ...
    if (t->leaf) {
        ...
        switch (t->leaf->category) {
            case INTEGERLITERAL:
            case LONGLITERAL:      t->type = integer_typeptr; return t->type;
            case REALLITERAL:
            case DOUBLELITERAL:    t->type = float_typeptr; return t->type;
            case BOOLEANLITERAL:   t->type = bool_typeptr; return t->type;
            case CHARACTERLITERAL: t->type = char_typeptr; return t->type;
            case STRINGLITERAL:    t->type = string_typeptr; return t->type;
            case NULLLITERAL:      t->type = null_typeptr; return t->type;
            case IDENT: {
