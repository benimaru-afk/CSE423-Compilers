...
/* ─────────────────────────────────────────────────────────────────────────
 * assign_offsets()
 *   Walk all entries in a symbol table and assign each one a memory address.
 *   region: R_LOCAL for function locals/params, R_GLOBAL for global scope.
 *   All values are 8 bytes (ints, pointers, refs all use 8 bytes per spec).
 * ───────────────────────────────────────────────────────────────────────── */
void assign_offsets(SymbolTable st, int region)
{
    if (!st) return;
    int offset = 0;
    for (int i = 0; i < st->nBuckets; i++) {
        SymbolTableEntry e = st->tbl[i];
        while (e) {
            e->place.region   = region;
            e->place.u.offset = offset;
            offset += 8;   /* all values are 8 bytes */
            e = e->next;
        }
    }
}
