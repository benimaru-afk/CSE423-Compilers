...
int main(int argc, char *argv[]) {
    ...
    int do_tree   = 0;
    int do_symtab = 0;
    int do_asm_only = 0;   /* -s: stop after writing .s */
    int do_obj_only = 0;   /* -c: assemble to .o but do not link */
    int first_file = -1;

    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-tree")   == 0) { do_tree = 1; debug_tokens = 1; continue; }
        if (strcmp(argv[i], "-symtab") == 0) { do_symtab = 1; continue; }
        if (strcmp(argv[i], "-s")      == 0) { do_asm_only = 1; continue; }
        if (strcmp(argv[i], "-c")      == 0) { do_obj_only = 1; continue; }
        if (first_file < 0) first_file = i;
    }

    if (first_file < 0) {
        fprintf(stderr, "usage: k0 [-s|-c] [-tree] [-symtab] <file.kt>\n");
        return 1;
    }
    ...
    for (int i = first_file; i < argc; i++) {
        ...
        if (semantic_errors > 0) {
            ...
            /* Generate intermediate and x86-64 assembly code */
            ...
            /* ── Post-assembly steps (unless -s) ───────────────────── */
            if (!do_asm_only) {
                /* Strip directory (output_x86 writes to cwd, not input dir) */
                const char *src_base = strrchr(argv[i], '/');
                src_base = src_base ? src_base + 1 : argv[i];
                const char *src_bs = strrchr(src_base, '\\');
                if (src_bs) src_base = src_bs + 1;

                /* Build base name: "foo.kt" → "foo" */
                char base[1024];
                strncpy(base, src_base, sizeof(base) - 1);
                base[sizeof(base) - 1] = '\0';
                char *dot = strrchr(base, '.');
                if (dot) *dot = '\0';

                /* Derive .s and .o filenames */
                char asm_file[1040], obj_file[1040];
                snprintf(asm_file, sizeof(asm_file), "%s.s", base);
                snprintf(obj_file, sizeof(obj_file), "%s.o", base);

                /* Step 1: assemble .s → .o */
                char as_cmd[2200];
                snprintf(as_cmd, sizeof(as_cmd), "as -o %s %s", obj_file, asm_file);
                int as_ret = system(as_cmd);

                /* Step 2: link .o → executable (only if no -c and assembly succeeded) */
                if (!do_obj_only && as_ret == 0) {
                    char ld_cmd[2200];
                    snprintf(ld_cmd, sizeof(ld_cmd),
                             "gcc -o %s %s", base, obj_file);
                    system(ld_cmd);
                }
            }
